...
The remainder of the paper is organized as follows. Section~\ref{sec:data} describes the data construction. Section~\ref{sec:strategy} presents the empirical strategy. Section~\ref{sec:results} reports the baseline and heterogeneity results. Section~\ref{sec:model} develops the structural model and policy evaluation. Section~\ref{sec:conclusion} concludes.
...
\begin{itemize}
    \item \textbf{Appendix A.} Robustness Checks (instrument weight variants; alternative commodity shocks; EU27 extension and EU7 placebo; lag sensitivity; sample stability)
...
\input{../output/tables/tab_rob_instruments.tex}
...
\input{../output/tables/tab_phase3_comparison.tex}
...
\paragraph{Sample stability: pre- and post-COVID sub-periods.}
\autoref{fig:lp_samples} overlays the full-sample LP-IV response with pre-2020 and post-2020 sub-period estimates, using the same instrument, controls, and EMU20 sample throughout. The ninety-percent confidence band is shown for the full-sample estimate only. The post-2020 period is characterised by energy price interventions, administered price caps, and supply disruptions that altered the normal relationship between global oil price shocks and euro area tradable inflation. All three sub-samples yield strong first-stage $F$-statistics: 208.6 (full), 132.9 (pre-2020), and 74.3 (post-2020), confirming that instrument weakness is not driving the sub-period differences.

\autoref{fig:lp_samples} reveals a marked divergence across sub-periods. The pre-2020 path peaks at approximately 0.48 at month six and declines to below zero by month thirteen, implying a pass-through roughly half the full-sample estimate at its peak. The post-2020 path is substantially larger: it rises from 0.11 at impact, crosses 1.0 by month six, and continues rising to a plateau near 1.4--1.5 through months ten to thirteen.

The full-sample estimate sits between the two, reflecting the average of a moderate pre-2020 regime and a substantially amplified post-2020 regime. This divergence is consistent with the state-dependence evidence in \autoref{fig:regimes}: the post-2020 period coincided precisely with above-normal GSCPI readings, sustained exchange rate depreciation, and elevated inflation volatility---the three conditions shown to amplify pass-through most strongly. The pre-2020 sample therefore provides the cleaner estimate of the long-run structural pass-through parameter, while the post-2020 results illustrate the conditions under which second-round effects can amplify substantially. Framed as state dependence rather than structural instability, the divergence reinforces rather than undermines the paper's central finding.
